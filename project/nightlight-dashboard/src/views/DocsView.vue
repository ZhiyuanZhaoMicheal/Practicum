...
const sections = [
  ...
  },
  {
    ...
    summary: 'How previous research used nighttime light data for disaster monitoring, and what gap our project addresses. Wang et al. (2018) demonstrated NASA Black Marble for outage mapping; Zhang et al. (2023) extended it to damage assessment. Our contribution: inverting the standard approach — detecting where lights stay on (backup power) rather than where they go out (damage).',
  ...
  },
  {
    ...
    summary: 'Four model variants (A–D) systematically test what drives prediction: Model A uses all features (AUC 0.967), Model B removes pre-NTL (AUC 0.968), Model C adds building footprints (AUC 0.966), and Model D uses pure NTL behavior only (AUC 0.704). The ensemble model generates per-pixel backup power probability maps for all 25 study areas, validated against Miami-Dade generator permit records.',
    tags: ['RF + XGB', 'LOEO', '4 Model Variants', 'AUC 0.967'],
  },
  {
    id: 'stage3', num: '08',
    title: 'Zip-Code Analysis',
    summary: 'Do areas with more critical facilities experience less severe power outages? This section extends the pixel-level analysis to zip-code-level spatial regression. Four models — OLS, Moran\'s I, Spatial Error Model, and subgroup analysis — test whether facility density predicts backup power probability across 1,002 zip codes and 22 U.S. disaster events. R² = 0.475, consistent across all city sizes and disaster types.',
